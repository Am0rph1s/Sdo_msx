...
// Boss sprite 16x16 - 2 layers (white hull + red core)
// 
// MSX 16x16 pattern format (32 bytes):
//  TL: rows 0-7 left half  (bytes 0-7)
//  BL: rows 8-15 left half (bytes 8-15)
//  TR: rows 0-7 right half (bytes 16-23)
//  BR: rows 8-15 right half (bytes 24-31)
// White layer (sprite color 7 = cyan)
// Red layer (sprite color 8 = medium red)

static const u8 g_BossWhiteNew[32] = {
    0x0F, 0x1F, 0x3C, 0x58, 0xFA, 0xEF, 0xFE, 0xF7,
    0xFD, 0xDF, 0xBF, 0x77, 0x6F, 0x1B, 0x17, 0x0E,
    0xF0, 0xF8, 0x3C, 0x1A, 0x5F, 0xF7, 0x7F, 0xEF,
    0xBF, 0xFB, 0xFD, 0xEE, 0xF6, 0xD8, 0xE8, 0x70
};

// Red layer: core nucleus + accents
static const u8 g_BossRedNew[32] = {
    0x00, 0x00, 0x00, 0x20, 0x00, 0x10, 0x01, 0x08,
    0x02, 0x20, 0x40, 0x08, 0x10, 0x04, 0x08, 0x01,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x08, 0x80, 0x10,
    0x40, 0x04, 0x02, 0x10, 0x08, 0x20, 0x10, 0x80
};
...
void UpdateHUD()
{
    ...
    // Boss HP bars: first at row 17, second (dual) at row 19
    {
        ...
        for (br = 0; br < boss_count; br++)
        {
            ...
            if (full || last_has_boss != boss_count || *last != bf)
                for (t = 0; t < 5; t++)
                    VDP_Poke_GM2((u8)(HUD_COL + 2 + t), row, (t < bf) ? BAR_FILL_TILE : 0);
            *last = bf;
        }
                ...
                if (g_GameOverDelay == 0 && g_GameState == GS_PLAYING)
               {
                 ...
                 for (j = 0; j < MAX_EXPLOSIONS && spr < 28; j++)
                 {
                    ...
                    // Enemies - WHITE layer first (shapes, high priority)
                  {
                      ...
                      static const u8 etype_col_w[5] = {COLOR_CYAN, COLOR_LIGHT_YELLOW, COLOR_GRAY, COLOR_LIGHT_GREEN, COLOR_WHITE};
